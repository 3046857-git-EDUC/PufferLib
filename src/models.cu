...
static void mingru_reg_train(void* w, void* activations, Allocator* acts, Allocator* grads, int B_TT) {
    ...
    for (int i = 0; i < m->num_layers; i++) {
        ...
        memset(scan, 0, sizeof(PrefixScan));
        scan->B = B;
        scan->T = TT;
        scan->H = H;
        scan->a_star.shape[0] = B;
        scan->a_star.shape[1] = TT + 1;
        scan->a_star.shape[2] = H;
        scan->s_vals.shape[0] = B;
        scan->s_vals.shape[1] = TT + 1;
        scan->s_vals.shape[2] = H;
        scan->log_values_buf.shape[0] = B;
        scan->log_values_buf.shape[1] = TT + 1;
        scan->log_values_buf.shape[2] = H;
        scan->out.shape[0] = B;
        scan->out.shape[1] = TT;
        scan->out.shape[2] = H;
        scan->next_state.shape[0] = B;
        scan->next_state.shape[1] = 1;
        scan->next_state.shape[2] = H;
        scan->grad_combined.shape[0] = B;
        scan->grad_combined.shape[1] = TT;
        scan->grad_combined.shape[2] = 3 * H;
        scan->grad_state.shape[0] = B;
        scan->grad_state.shape[1] = 1;
        scan->grad_state.shape[2] = H;
        scan->grad_input.shape[0] = B;
        scan->grad_input.shape[1] = TT;
        scan->grad_input.shape[2] = H;
